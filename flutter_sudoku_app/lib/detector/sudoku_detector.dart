...
class SudokuDetector {
  ...
  init() async {
    ...
    final bytes = await rootBundle.load('assets/fakepath');
  ...
  Float32List? detect(CameraImage image, int rotation) {
    ...
    in iOS the form is BGRA and we get a single buffer for all channels. 
    So the yBuffer variable on Android will be just the Y channel but on iOS it 
    will be the entire image */
    ...
    var res = _nativeOpencv!
        .detect(image.width, image.height, rotation, yBuffer, uBuffer, vBuffer);
