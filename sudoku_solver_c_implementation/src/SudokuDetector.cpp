...
int sudoku::detectSudokuPuzzle(const char *image_path, Mat &final_img, bool display /* false */) {
    Mat original_image, colored_image;
    original_image = imread(image_path, IMREAD_GRAYSCALE);
    colored_image = imread(image_path, IMREAD_COLOR);
    int img_width = original_image.cols; int img_height = original_image.rows;
    ...
    medianBlur(original_image, modified_image, 3);
